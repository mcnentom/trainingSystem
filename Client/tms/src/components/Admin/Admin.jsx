...
function AdminDashboard() {
  ...
  const [newBatchHeader, setNewBatchHeader] = useState("");
  ...
  async function fetchData() {
    try {
      const response = await axios.get(
        "http://localhost:3000/adminActions/users",
  ...
  async function fetchCourses() {
    try {
      const coursesResponse = await axios.get(
        "http://localhost:3000/adminActions/courses",
        { withCredentials: true }
      );
  ...
  const handleRegistration = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:3000/adminActions/register",
        {
          method: "POST",
          body: formData,
        }
  ...
  const handleDeleteUser = async (email) => {
    try {
      const response = await axios.delete(
        `http://localhost:3000/adminActions/users/${email}`,
        { withCredentials: true }
      );
  ...
  const handleDeleteCourse = async (courseId) => {
    try {
      const response = await axios.delete(
        `http://localhost:3000/adminActions/courses/${courseId}`,
        { withCredentials: true }
      );
    ...
    <div className="Admin">
      ...
      <div className="AdminDiv">
        <div className="sidebar">
          <ul>
            <li onClick={() => handleTabClick("user")} className="mylist">
              User Management
            </li>
            <li onClick={() => handleTabClick("course")} className="mylist">
              Course Management
            </li>
            <li onClick={() => handleTabClick("content")} className="mylist">
              User Registration
            </li>
            <li
              onClick={() => {
          ...
          {activeTab === "course" && (
            <div>
              ...
              <form onSubmit={addCourse} className="CourseForm">
                ...
                <input
                  type="text"
                  value={newBatchHeader}
                  onChange={(e) => setNewBatchHeader(e.target.value)}
                  required
                  className="BatchHeader"
                />

                <label>Content</label>
                <textarea
                  value={newMaterialTypes}
                  onChange={(e) => setNewMaterialTypes(e.target.value)}
                  required
                  className="MaterialTypesField"
                />
                ...
                <tbody>
                  {userCourses.map((course) => (
                    <tr key={course.course_id}>
                      ...
                      <td>
                        <ul>
                          {course.materialBatches.map((batch) => (
                            <li key={batch.batch_number} className="batchList">
                              <strong className="batchHeader"> {batch.batch_number}</strong>
                             
                              <br />
                              <p className="batchContent">{batch.material_types}</p>
